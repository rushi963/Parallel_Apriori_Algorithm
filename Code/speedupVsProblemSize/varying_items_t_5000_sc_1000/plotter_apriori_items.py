...
file = open('threads_16.txt')	#change file name accordingly.
speedup5 = []
st5 = []
pt5 = []
for i in range(8):
...
plt.plot(sizes,speedup5,label = '16 threads')
plt.legend(loc = 'upper left')
plt.ylabel('speedup')
#plt.ylim(0,8)
#plt.xscale('log')
plt.xlabel('No of items')
plt.title('speedup vs No of items(t=5000,sc=1000)')
plt.figure();
plt.show();
...
plt.plot(sizes,pt5,label = 'parallel_16_threads')
plt.legend(loc = 'upper left')
#plt.xscale('log')
plt.ylabel('time required in sec')
plt.xlabel('No of items')
plt.title('Time required vs No of items(t=5000,sc=1000)	')
plt.show()
